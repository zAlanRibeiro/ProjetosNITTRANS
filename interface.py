...
from TarjarPDF.interface_tarjar import JanelaHigienizar
...
DICAS = {
    "enderecos.py": (
        "Converte coordenadas geográficas em endereços completos via"
        " OpenStreetMap."
    ),
    "limpeza.py": (
        "Corrige encoding, padroniza logradouros e valida endereços em"
        " planilhas."
    ),
    "decifradorTxt.py": (
        "Converte arquivos .txt posicionais do DETRAN/RJ em planilha Excel."
    ),
    "detranLimpo.py": (
        "Limpa coluna de endereço removendo números, OP., OPOSTO e cruzamentos."
    ),
    "pdfDeferidoIndeferido.py": (
...
class HubApp(ctk.CTk):
  ...
  def _construir_interface(self):
    ...
    # ── IMAGEM DE FUNDO ───────────────────────────────────────────────────
    for caminho_fundo in [
        os.path.join(base, "FundoAplicativoNITTRANS.png"),
    ...
    for ext in ["jpeg", "jpg", "png"]:
      for caminho in [
          os.path.join(base, f"LogoNittrans.{ext}"),
    ...
    self.canvas.create_text(
        230,
        125,
        text="Departamento de Gestão e Modernização",
        font=(FONTE, 15, "bold"),
        fill="#FFFFFF",
        anchor="center",
    )
    ...
    btn_t = ctk.CTkButton(
        ...
        text_color="#FFFFFF",
        bg_color=COR_VIDRO_BLENDED,
        command=self._abrir_tarjar,
    )
    btn_t.place(x=20, y=sep_y + 80)
    ...
    btn_b = ctk.CTkButton(
        ...
        text_color="#FFFFFF",
        bg_color=COR_VIDRO_BLENDED,
        command=self._abrir_bloqueio_planilha,
    )
    btn_b.place(x=20, y=sep_y + 125)
    _Tooltip(btn_b, DICAS["bloquear_planilha"])
  ...
  def _abrir_bloqueio_planilha(self):
    from BloquearPlanilha.gui import AppToplevel
  ...
  def _exibir_sucesso(self, pasta):
    ...
    self.canvas.itemconfig(
        self.id_status, text=f"✔  Concluído em {tempo} — {pasta}", fill="#4ADE80"
    )
  ...
  def _exibir_erro(self, erro):
    ...
    self.canvas.itemconfig(
        self.id_status, text=f"✖  Erro ao processar:\n{resumo}", fill="#F87171"
    )
    self.btn_exportar.place_forget()
