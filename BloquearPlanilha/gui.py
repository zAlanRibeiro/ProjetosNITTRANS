...
try:
  from BloquearPlanilha.bloqueador import bloquear_e_formatar_planilha
except ImportError:
  from bloqueador import bloquear_e_formatar_planilha

# ── Paleta idêntica ao Hub e Criptografia ──────────────────────────────────
...
class _AppMixin:
  """Lógica e UI compartilhadas para Bloquear Planilhas."""
  ...
  def _processar(self):
    ...
    self.canvas.itemconfig(
        self.id_status, text="Aplicando formatação e bloqueio…", fill=_TEXTO_S
    )

    def tarefa():
      try:
        bloquear_e_formatar_planilha(caminho, destino, senha, modo_bloqueio="total")
        ...
        self.after(
            ...
            lambda: messagebox.showinfo(
                "Sucesso",
                f"Planilha bloqueada e formatada com sucesso!\n\nSalva em:\n"
                f"  {destino}",
            ),
        )
      except Exception as e:
